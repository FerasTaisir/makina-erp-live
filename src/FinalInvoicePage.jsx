...
function getNextFinalInvoiceRef(existingRefs) {
  let maxNumber = 0;

  (existingRefs || []).forEach((row) => {
    const code = String(row?.final_invoice_ref || "");
    const match = code.match(/FI-(\d+)/i);
  ...
  return `FI-${String(maxNumber + 1).padStart(6, "0")}`;
}
...
export default function FinalInvoicePage() {
  ...
  async function loadData() {
    try {
      ...
      const [
        ...
        supabase.from("shipping_lines").select("*").order("line_no", { ascending: true }),
        ...
        supabase.from("final_invoice_headers").select("final_invoice_ref"),
      ]);

      if (shippingsRes.error) throw shippingsRes.error;
      ...
      setShippingLines(linesRes.data || []);
      ...
      setInvoiceRef(getNextFinalInvoiceRef(refsData));

      const selectedFromOrder = localStorage.getItem("selected_shipping_id");
      if (selectedFromOrder) {
        setForm((prev) => ({
          ...prev,
          shipping_id: selectedFromOrder,
        }));
        localStorage.removeItem("selected_shipping_id");
      }
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...prev,
      offer_type: prev.offer_type || getFirstDefinitionValue(invoiceDefinitions, "invoice_title") || "Final Invoice",
  ...
  const selectedShipping = useMemo(() => {
    return (
      shippings.find((row) => String(row.id) === String(form.shipping_id)) || null
    );
  }, [shippings, form.shipping_id]);

  const selectedCustomer = selectedShipping
    ? customerMap[String(selectedShipping.customer_id)] || null
    : null;

  const selectedShippingLines = useMemo(() => {
    if (!form.shipping_id) return [];

    return shippingLines.filter(
      (row) => String(row.shipping_header_id) === String(form.shipping_id)
    );
  }, [shippingLines, form.shipping_id]);
      ...
      formulaRows.find(
        (row) =>
          String(row.customer_id || "") ===
            String(selectedShipping?.customer_id || "") &&
  ...
  const invoiceLines = useMemo(() => {
    return selectedShippingLines.map((line, index) => {
      ...
      const qty = num(line.shipped_qty);
  ...
  function clearForm() {
    ...
    setInvoiceRef(getNextFinalInvoiceRef(invoiceRefs));
    setMessage("");
    setError("");
  }

  async function handleSaveFinalInvoice() {
    if (!form.shipping_id) {
      setError("Please select Shipping first.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        final_invoice_ref: invoiceRef,
        final_invoice_date: invoiceDate,
        shipping_header_id: form.shipping_id,
        customer_id: selectedShipping?.customer_id || null,
      ...
      const linesPayload = invoiceLines.map((line) => ({
        final_invoice_header_id: header.id,
        line_no: line.no,
        item_name: line.item,
        ...
        unit_usd: convertAmount(line.unitPrice),
        total_usd: convertAmount(line.totalUsd),
      }));

      if (linesPayload.length > 0) {
        const { error: linesError } = await supabase
          .from("final_invoice_lines")
          .insert(linesPayload);

        if (linesError) throw linesError;
      }

      setMessage("Final Invoice saved successfully.");
      await loadData();
    } catch (err) {
      console.error(err);
      setError(err.message || "Failed to save Final Invoice.");
    } finally {
  ...
  return (
    <div className="invoice-page">
      <div className="no-print page-title-wrap">
        <h1>Final Invoice</h1>
        <p>Create Final Invoice from shipped quantities</p>
      </div>
      ...
      <div className="invoice-paper">
        ...
        <div className="system-note">
          This is system-generated Final Invoice and does not require any signature or stamp.
        </div>
      </div>
      <div className="no-print bottom-invoice-controls">
        <div className="bottom-controls-grid">
          <div className="form-group">
            ...
            <select
              ...
              onChange={(e) => handleChange("shipping_id", e.target.value)}
            >
              ...
              {shippings.map((order) => {
                ...
                return (
                  <option key={order.id} value={order.id}>
                    {order.shipping_ref || `Shipping #${order.id}`} - {customer.customer_name || ""}
                  </option>
                );
              })}
            </select>
        ...
        <div className="actions bottom-actions">
          <button type="button" onClick={handleSaveFinalInvoice} disabled={savingInvoice}>
            {savingInvoice ? "Saving..." : "Save Final Invoice"}
          </button>
