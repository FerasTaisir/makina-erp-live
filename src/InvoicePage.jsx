...
function getNextInvoiceRef(existingRefs) {
  let maxNumber = 0;

  (existingRefs || []).forEach((row) => {
    const code = String(row?.invoice_ref || "");
    const match = code.match(/PI-(\d+)/i);
  ...
  return `PI-${String(maxNumber + 1).padStart(6, "0")}`;
}
...
export default function InvoicePage() {
  ...
  async function loadData() {
    try {
      ...
      const [
        ...
        supabase.from("invoice_headers").select("invoice_ref"),
      ]);
      ...
      setInvoiceRef(getNextInvoiceRef(refsData));
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...prev,
      offer_type: prev.offer_type || getFirstDefinitionValue(invoiceDefinitions, "offer_type") || getFirstDefinitionValue(invoiceDefinitions, "invoice_title"),
  ...
  function clearForm() {
    ...
    setInvoiceRef(getNextInvoiceRef(invoiceRefs));
    setMessage("");
    setError("");
  }
  ...
  return (
    <div className="invoice-page">
      <div className="no-print page-title-wrap">
        <h1>Invoice</h1>
        <p>Create Performa Invoice from saved Order</p>
      </div>
      ...
      <div className="invoice-paper">
        ...
        <div className="invoice-title">
          ...
          >
            <option value="">Select Invoice Title</option>
            {[
